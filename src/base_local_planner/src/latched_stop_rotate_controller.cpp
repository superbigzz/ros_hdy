...
bool LatchedStopRotateController::isPositionReached(LocalPlannerUtil* planner_util,
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    ROS_INFO("xy_tolerance_latch_ = true");
...
bool LatchedStopRotateController::isGoalReached(LocalPlannerUtil* planner_util,
  ...
  ROS_INFO("latched_stop_rotate: isGoalReached?");
  ...
  tf::Stamped<tf::Pose> goal_pose;
  if ( ! planner_util->getGoal(goal_pose)) {
    ROS_INFO("! planner_util->getGoal(goal_pose)");
    return false;
  }
  ...
  if ((latch_xy_goal_tolerance_ && xy_tolerance_latch_) ||
    ...
    if (fabs(angle) <= limits.yaw_goal_tolerance) {
      ...
      ROS_INFO("fabs(angle) <= limits.yaw_goal_tolerance");
  ...
  ROS_INFO("not reached the position");
  return false;
}
...
bool LatchedStopRotateController::stopWithAccLimits(const tf::Stamped<tf::Pose>& global_pose,
  ...
  if(valid_cmd){
    ...
    ROS_INFO("latched_stop_rotate, Slowing down... using vx, vy, vth: %.2f, %.2f, %.2f", vx, vy, vth);
    cmd_vel.linear.x = vx;
    cmd_vel.linear.y = vy;
...
bool LatchedStopRotateController::computeVelocityCommandsStopRotate(geometry_msgs::Twist& cmd_vel,
  ...
  if (fabs(angle) <= limits.yaw_goal_tolerance) {
    ...
    ROS_INFO("the goal orientation has been reached");
    cmd_vel.linear.x = 0.0;
    cmd_vel.linear.y = 0.0;
    cmd_vel.angular.z = 0.0;
    rotating_to_goal_ = false;
  } else {
    ROS_INFO("the goal orientation has not been reached. Angle: %f Tolerance: %f", angle, limits.yaw_goal_tolerance);
